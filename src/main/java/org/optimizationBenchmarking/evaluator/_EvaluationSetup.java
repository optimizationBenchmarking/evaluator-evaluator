...
import org.optimizationBenchmarking.evaluator.evaluation.impl.abstr.DocumentEvaluationOutput;
import org.optimizationBenchmarking.evaluator.evaluation.impl.abstr.ExperimentSetEvaluationInput;
...
class _EvaluationSetup {
  ...
  static final int TYPE_EXPERIMENT = (_EvaluationSetup.TYPE_DESCRIPTION
      + 1);
  /** the module is an experiment module */
  static final int TYPE_EXPERIMENT_SET = (_EvaluationSetup.TYPE_EXPERIMENT
      + 1);
  /** the module is an appendix module */
  static final int TYPE_APPENDIX = (_EvaluationSetup.TYPE_EXPERIMENT_SET
      + 1);
  ...
  void _assign(final _EvaluationSetup copyFrom) {
    ...
    synchronized (this.m_synch) {
      synchronized (copyFrom.m_synch) {
        ...
        for (final ArrayList<_ModuleEntry> list : copyFrom
            ._takeModules()) {
   ...
   * {@link org.optimizationBenchmarking.evaluator.evaluation.spec.IEvaluationBuilder#setOutput(IEvaluationOutput)}
   * method taking an instance of
   * {@link org.optimizationBenchmarking.evaluator.evaluation.spec.IEvaluationOutput}
   ...
   *          files
   */
  final void _setFileProducerListener(
      final IFileProducerListener listener) {
    synchronized (this.m_synch) {
      this.m_fpo = listener;
    }
  ...
  final void _addModule(final IEvaluationModule module,
    ...
    synchronized (this.m_synch) {
      ...
      list.add(
          new _ModuleEntry(module, this._getConfiguration(config), size));
    }
  }
  ...
  static final void _checkDocument(final IDocument doc) {
    if (doc == null) {
      throw new IllegalArgumentException(
          "Output document cannot be null."); //$NON-NLS-1$
    }
  }
   ...
   *          the output
   */
  static final void _checkEvaluationOutput(
      final IEvaluationOutput output) {
    if (output == null) {
      throw new IllegalArgumentException(//
